...
Данная библиотека содержит следующие возможности:
...
**`Данная библиотека поддерживается всеми ПЛК, программируемыми в среде Codesys 3  или IDE разработанных на его основе`**<br/>
...
|Value    |Values |Структура  хранящая значение **`(Структура приведена ниже)`**   |
|vType   |isTypes  |Перечисление определяющего тип свойства **`(Структура приведена ниже)`**|
...
POST-запрос осуществляется с помощью функционалного блока `IIOT.HTTP_POST`, данный блок имеет следующие параметры:
